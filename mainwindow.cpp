    ...
    , exec(new Turing::Executer)
{
    ...
    this->_timer = new QTimer(this); // Crea un objeto de la clase QTimer y pasa la dirección de la variable
    this->_timer->setInterval(10); // Establecer el intervalo del temporizador
    connect(this->_timer, SIGNAL(timeout()), this, SLOT(update_time())); // Conectamos la señal del temporizador a nuestra ranura.
    this->_timer->start(); // Iniciar el temporizador
...
void MainWindow::paintEvent(QPaintEvent *event) // Representación de la cinta
{
    Q_UNUSED(event);
...
void MainWindow::update_table() { // Actualización de tabla
    this->_no_edit = true;
...
void MainWindow::update_time() //Temporizador básico
{
    QWidget* left = this->ui->horizontalLayout_2->itemAt(0)->widget();
    auto window = this->geometry();
...
void MainWindow::resizeEvent(QResizeEvent* event) // Evento de cambio de tamaño de ventana
{
    QMainWindow::resizeEvent(event);
    this->repaint();
}

void MainWindow::closeEvent(QCloseEvent* event) //Comprobar si hay datos no guardados
{
    if (!this->_changed) {
        event->accept();
        return;
    ...
    QMessageBox messageBox(QMessageBox::Question,
                tr("Turing"),
                tr("В El archivo actual tiene cambios sin guardar. ¿De verdad quieres salir?"),
                QMessageBox::Default,
                this);
    messageBox.addButton("Sí", QMessageBox::YesRole);
...
void MainWindow::update_from_settings() { // Mover datos de la configuración al formulario
...
void MainWindow::save_into_settings() { // Transferir datos del formulario a la configuración
...
void MainWindow::set_speed(int value) { // int -> Velocidad
    switch(value) {
    case 0:
...
int MainWindow::get_speed() { // Velocidad -> int
...
void MainWindow::on_action_15_toggled(bool state) // Velocidad -> Muy rapido
{
    if (state) {
...
void MainWindow::on_action_16_toggled(bool state) // Velocidad -> Rápido
{
    if (state) {
...
void MainWindow::on_action_17_toggled(bool state) // Velocidad -> Media
{
    if (state) {
        this->ui->action_16->setChecked(false);
        this->ui->action_15->setChecked(false);
...
void MainWindow::on_action_18_toggled(bool state) // Velocidad -> Muy lenta
{
    if (state) {
        this->ui->action_16->setChecked(false);
        this->ui->action_17->setChecked(false);
...
void MainWindow::on_action_19_toggled(bool state) // Velocidad -> Muy lenta
{
    if (state) {
        this->ui->action_16->setChecked(false);
        this->ui->action_17->setChecked(false);
...
void MainWindow::on_action_triggered() // Archivo -> Nuevo
{
    this->exec->get_settings()->clear();
    this->update_from_settings();
    this->setWindowTitle("Turing");
}

void MainWindow::on_action_2_triggered() // Файл -> Открыть...
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Apertura"), "", tr("Ajustes de Turing (*.tur)"));
    if (!this->exec->get_settings()->open(fileName)) {
        QMessageBox::warning(this, "Turing", "В Ocurrió un error durante el proceso de apertura. El archivo puede estar dañado.");
    } else {
        this->update_from_settings();
        pointer_reset();
        this->setWindowTitle("Turing (" + fileName + ")");
    }
}

void MainWindow::on_action_3_triggered() // Archivo -> Guardar
{
    this->save_into_settings();
    if (!this->exec->get_settings()->save()) {
        QMessageBox::warning(this, "Turing", "В Ocurrió un error al guardar. Tal vez, " \
                                 "está intentando guardar un archivo en una ruta vacía. En ese caso, utilice \"Guardar como...\"");
    } else {
        this->_changed = false;
    }
}
...
void MainWindow::on_action_4_triggered() // Archivar -> Guardar como...
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Guardar"), "", tr("Turing settings (*.tur)"));
    this->save_into_settings();
...
void MainWindow::on_problemBox_textChanged() // La tarea
{
    this->_changed = true;
}


void MainWindow::on_input_alphabetBox_textEdited(const QString &text) // Alfabeto fuente
{
...
void MainWindow::on_additional_alphabetBox_textEdited(const QString &text) //Alfabeto adicional
{
...
void MainWindow::on_commentsBox_textChanged() // Comentarios
{
    this->_changed = true;
}
...
void MainWindow::on_action_11_triggered() // Cinta -> Establecer cadena
{
...
void MainWindow::on_action_5_triggered() // Cinta -> Guardar
{
...
void MainWindow::on_action_6_triggered() // Cinta -> Restaurar
{
...
void MainWindow::on_addRowBtn_clicked() // Agregar
{
    this->_no_edit = true;
...
void MainWindow::on_delRowBtn_clicked() // Borrar
{
    this->_no_edit = true;
...
void MainWindow::on_tableWidget_cellChanged(int row, int column)
{
    ...
    if (!this->exec->get_table()->change_action(row, actions)) {
        this->_no_edit = true;
        QMessageBox::warning(this, "Turing", "Formato de tabla incorrecto.");
...
void MainWindow::on_action_8_triggered()
{
    ...
    if (action.get_symbol() == '\0' && action.get_move() == Turing::Action::None && action.get_next_state() == -2) {
        this->_run = false;
        set_enabled(true);
        QMessageBox::warning(this, "Turing", "Error al llenar la tabla. La celda actual está vacía.");
    } else {
